-- Dealer profit estimate (best case): use Finance-provided est_lifetime_margin_eur on contracts
WITH contract_margin AS (
  SELECT
    ...
    SUM(c.est_lifetime_margin_eur) AS total_margin_eur
...
SELECT
  ...
  cm.num_customers,
  cm.total_volume,
  cm.total_margin_eur AS net_profit_estimate
...
ORDER BY cm.total_margin_eur DESC;
